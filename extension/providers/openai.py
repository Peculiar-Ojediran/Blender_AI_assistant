...
an earlier CREATE_PRIMITIVE, CREATE_MATERIAL, ADD_LIGHT, or ADD_CAMERA operation as
result:<operation_id>. Never use a forward result reference. Copy the scene context
snapshot_id into the plan snapshot_id exactly. File reads, file writes, external asset
downloads, subprocesses, and generated Python execution are unsupported. Never propose a
workaround for those capabilities; return needs_clarification and explain that the request is
outside the controlled operation contract."""
