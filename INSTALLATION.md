...
- Internet access to the selected provider endpoint when planning a request.
- An OpenAI or NVIDIA API key with available API access.
...
The safest supported source is the operating-system environment. For OpenAI:
...
Restart Blender after setting it. The extension checks key sources for the selected provider in this
order:

1. Operating-system `OPENAI_API_KEY` or `NVIDIA_API_KEY`.
...
5. Choose a provider and model from the dropdowns.
...
files or provider account data.
